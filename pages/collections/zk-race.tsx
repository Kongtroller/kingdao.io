...
const ZKRaceCollection = () => {
  ...
  useEffect(() => {
    ...
    racers.forEach((racer: ZKRaceNFT) => {
      racer.traits.forEach((trait: Trait) => {
  ...
  useEffect(() => {
    if (inView) {
      setDisplayedCount(prev => 
        Math.min(prev + ITEMS_PER_PAGE, filteredRacers.length)
      );
    }
  }, [inView]);

  const filteredRacers = racers.filter((racer: ZKRaceNFT) => 
    racer.tokenId.includes(searchQuery) || 
    racer.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const displayedRacers = filteredRacers.slice(0, displayedCount);
    ...
    <CollectionLayout collection={collection}>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8">
        <div className="text-sm text-gray-600 dark:text-gray-400 mb-4 md:mb-0">
          Showing {displayedRacers.length} of {filteredRacers.length} NFTs
        </div>
      ...
      <div className={`grid gap-6 ${view === 'grid' ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1'}`}>
        {displayedRacers.map((racer: ZKRaceNFT) => (
          <div 
            key={racer.tokenId}
          ...
          >
            ...
            <div className={`${view === 'grid' ? 'p-4' : 'flex-1'}`}>
              ...
              {racer.traits.length > 0 && (
                <div className="mt-2">
                  <button 
                    onClick={() => setExpandedTraits(prev => ({
                      ...prev,
                      [racer.tokenId]: !prev[racer.tokenId]
                    }))}
                  ...
                  {expandedTraits[racer.tokenId] && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {racer.traits.map((trait: Trait, index: number) => (
                        <span 
                          key={index}
      ...
      {displayedCount < filteredRacers.length && (
        <div
          ref={ref}
...
export default ZKRaceCollection; 