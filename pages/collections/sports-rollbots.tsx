...
const SportsRollbotsCollection = () => {
  ...
  useEffect(() => {
    ...
    rollbots.forEach((bot: SportsRollbotNFT) => {
      bot.traits.forEach((trait: Trait) => {
  ...
  useEffect(() => {
    if (inView) {
      setDisplayedCount(prev => 
        Math.min(prev + ITEMS_PER_PAGE, filteredRollbots.length)
      );
    }
  }, [inView]);

  const filteredRollbots = rollbots.filter((bot: SportsRollbotNFT) => 
  ...
  return (
    <CollectionLayout collection={collection}>
      ...
      <div className={`grid ${view === 'grid' ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-4' : 'grid-cols-1'} gap-6`}>
        {displayedRollbots.map((rollbot: SportsRollbotNFT) => (
          <div 
            key={rollbot.tokenId}
                ...
                <div className="mt-2">
                  ...
                  {expandedTraits[rollbot.tokenId] && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {rollbot.traits.map((trait: Trait, index: number) => (
                        <span 
                          key={index}
...
export default SportsRollbotsCollection; 